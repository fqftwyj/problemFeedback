...
public class OfficeController extends BaseController<Office>{
	...
	@ResponseBody
	public Result index(Office office, ModelMap map,HttpSession session,Integer page,Integer limit){
		Map<String,Object> search=new HashMap<String,Object>();
		PageInfo<Office> pageinfo = officeService.findByPage(search,ProjectDefined.DEFAULT_ORDER_BY,(page==null?1:page),(limit==null?99999:limit));
	...
	@ResponseBody
	public Result create(Office office){
		if(office != null){
			Map<String,Object> map=new HashMap<String,Object>();
			/**
			 * 放入查重字段 map.put("name","测试");
			 */
			Integer flag=officeService.save(office,map,OperatorEnum.AND);
			if(flag==2) {
	...
	public void updateJump(Integer id, ModelMap map){
		Office result = officeService.find(id);
	...
	public Result update(Office office){ 
		if(office != null&&office.getId()!=null){
			 ...
			 * 放入查重字段 map.put("name","测试");
			 */
			Integer flag = officeService.update(office,map,OperatorEnum.AND);
			if(flag==2) {
				return ResultUtil.error("已存在");
			}else if(flag==1) {
	...
	public Result delete(String ids){
		...
		Integer flag=officeService.delete(idsall);
		if(flag==idsall.size()) {
	...
	@ResponseBody
	public String export(Office office,HttpSession session,
			HttpServletRequest request, HttpServletResponse response,@NotNull Integer page) {
		Map<String,Object> search=new HashMap<String,Object>();
		List<Map<String, Object>> list = officeService.excelExportList(search,ProjectDefined.DEFAULT_ORDER_BY,page,9999);
	...
	public Result callPro() {
		PageInfo<Office> pageinfo = officeService.findListByProceAndPage("pro", 1);
		return ResultUtil.success("查询成功", (int)pageinfo.getTotal(), pageinfo.getList());
	}
	*/
}
