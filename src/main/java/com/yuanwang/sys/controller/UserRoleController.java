...
public class UserRoleController extends BaseController<UserRole>{
	...
	@ResponseBody
	public Result index(UserRole userRole, ModelMap map,HttpSession session,Integer page,Integer limit){
		Map<String,Object> search=new HashMap<String,Object>();
		PageInfo<UserRole> pageinfo = userRoleService.findByPage(search,ProjectDefined.DEFAULT_ORDER_BY,(page==null?1:page),(limit==null?99999:limit));
	...
	public Result create(UserRole userRole){
		if(userRole != null){
			 ...
			 */
			Integer flag=userRoleService.save(userRole,map,OperatorEnum.AND);
			if(flag==2) {
	...
	public void updateJump(Integer id, ModelMap map){
		UserRole result = userRoleService.find(id);
	...
	public Result update(UserRole userRole){ 
		if(userRole != null && userRole.getId()!=null){
			 ...
			 * 放入查重字段 map。put("name","测试");
			 */
			Integer flag = userRoleService.update(userRole,map,OperatorEnum.AND);
			if(flag==2) {
				return ResultUtil.error("已存在");
			}else if(flag==1) {
	...
	public Result delete(String ids){
		...
		Integer flag=userRoleService.delete(idsall);
		if(flag==idsall.size()) {
	...
	@ResponseBody
	public String export(UserRole userRole,HttpSession session,
			HttpServletRequest request, HttpServletResponse response,@NotNull Integer page) {
		Map<String,Object> search=new HashMap<String,Object>();
		List<Map<String, Object>> list = userRoleService.excelExportList(search,ProjectDefined.DEFAULT_ORDER_BY,page,9999);
	...
	public Result callPro() {
		PageInfo<UserRole> pageinfo = userRoleService.findListByProceAndPage("pro", 1);
		return ResultUtil.success("查询成功", (int)pageinfo.getTotal(), pageinfo.getList());
	}
	*/
}
