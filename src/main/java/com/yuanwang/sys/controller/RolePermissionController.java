...
public class RolePermissionController extends BaseController<RolePermission>{
	...
	@ResponseBody
	public Result index(RolePermission rolePermission, ModelMap map,HttpSession session,Integer page,Integer limit){
		Map<String,Object> search=new HashMap<String,Object>();
		PageInfo<RolePermission> pageinfo = rolePermissionService.findByPage(search,ProjectDefined.DEFAULT_ORDER_BY,(page==null?1:page),(limit==null?99999:limit));
	...
	@ResponseBody
	public Result create(RolePermission rolePermission){
		if(rolePermission != null){
			Map<String,Object> map=new HashMap<String,Object>();
			/**
			 * 放入查重字段 map。put("name","测试");
			 */
			Integer flag=rolePermissionService.save(rolePermission,map,OperatorEnum.AND);
			if(flag==2) {
	...
	public void updateJump(Integer id, ModelMap map){
		RolePermission result = rolePermissionService.find(id);
	...
	public Result update(RolePermission rolePermission){ 
		if(rolePermission != null&&rolePermission.getId()!=null){
			 ...
			 * 放入查重字段 map。put("name","测试");
			 */
			Integer flag = rolePermissionService.update(rolePermission,map,OperatorEnum.AND);
			if(flag==2) {
				return ResultUtil.error("已存在");
			}else if(flag==1) {
	...
	public Result delete(String ids){
		...
		Integer flag=rolePermissionService.delete(idsall);
		if(flag==idsall.size()) {
	...
	@ResponseBody
	public String export(RolePermission rolePermission,HttpSession session,
			HttpServletRequest request, HttpServletResponse response,@NotNull Integer page) {
		Map<String,Object> search=new HashMap<String,Object>();
		List<Map<String, Object>> list = rolePermissionService.excelExportList(search,ProjectDefined.DEFAULT_ORDER_BY,page,9999);
	...
	public Result callPro() {
		PageInfo<RolePermission> pageinfo = rolePermissionService.findListByProceAndPage("pro", 1);
		return ResultUtil.success("查询成功", (int)pageinfo.getTotal(), pageinfo.getList());
	}
	*/
}
